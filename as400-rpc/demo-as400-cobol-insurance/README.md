...
1. Copy the cobol resources from - [resources](./assets/resources) folder into `src/main/resources/sample` folder.
...
>_NOTE_: The tests should run in the following order otherwise they won't pass. 
1. **Get All Policies**
...
6. Generate services from the SDK.
  - **Get All Policies**:
     ...
     - Select from `POL2OUTL` model the `policyOut` as output. 
     - **Click OK**
  - **Add Policy**:
     - **Right-Click** on the **policy-as400-cobol-api** project → OpenLegacy → Generate API from SDK
     - Name The service `AddPolicy`
     - Select from `POL2ADD` model the `pol2addInput` as input. 
     - Select from `POL2ADD` model the `pol2addOutput` as output. 
     - **Click OK**
  - **Upadte Policy**:
     - **Right-Click** on the **policy-as400-cobol-api** project → OpenLegacy → Generate API from SDK
     - Name The service `UpdatePolicy`
     - Select from `POL2UPT` model the `pol2updInput` as input. 
     - Select from `POL2UPT` model the `pol2updOutput` as output. 
     - **Click OK**
  - **Get Policy**:
     - **Right-Click** on the **policy-as400-cobol-api** project → OpenLegacy → Generate API from SDK
     - Name The service `GetPolicy`
     - Select from `POL2GET` model the `pol2getInput` as input. 
     - Select from `POL2GET` model the `pol2getOutput` as output. 
     - **Click OK**
  - **Get Policy**:
     - **Right-Click** on the **policy-as400-cobol-api** project → OpenLegacy → Generate API from SDK
     - Name The service `DeletePolicy`
     - Select from `POL2DEL` model the `pol2delInput` as input. 
     - Select from `POL2DEL` model the `pol2delOutput` as output. 
     - **Click OK**
